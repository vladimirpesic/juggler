...
impl MemoryRouter {
    pub fn new() -> Self {
        ...
        let instructions = formatdoc! {r#"
             ...
             3. Upon agreement:
                ...
                - Confirm the desired storage location:
                  - Local storage (.juggler/memory) for project-specific details.
                  - Global storage (~/.config/juggler/memory) for user-wide data.
        ...
        // Check for .juggler/memory in current directory
        let local_memory_dir = std::env::var("JUGGLER_WORKING_DIR")
            .map(PathBuf::from)
        ...
        // - macOS/Linux: ~/.config/juggler/memory/
        // - Windows:     ~\AppData\Roaming\juggler\config\memory
        // if it fails, fall back to `.config/juggler/memory` (relative to the current dir)
        let global_memory_dir = choose_app_strategy(crate::APP_STRATEGY.clone())
            .map(|strategy| strategy.in_config_dir("memory"))
            .unwrap_or_else(|_| PathBuf::from(".config/juggler/memory"));
