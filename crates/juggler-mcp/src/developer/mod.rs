...
impl DeveloperRouter {
    pub fn new() -> Self {
        ...
        // - macOS/Linux: ~/.config/juggler/
        // - Windows:     ~\AppData\Roaming\juggler\config\
        ...
        let global_hints_path = choose_app_strategy(crate::APP_STRATEGY.clone())
            .map(|strategy| strategy.in_config_dir(".jugglerhints"))
            .unwrap_or_else(|_| {
                PathBuf::from(shellexpand::tilde("~/.config/juggler/.jugglerhints").to_string())
            });
        ...
        let local_hints_path = cwd.join(".jugglerhints");
        ...
        let global_ignore_path = choose_app_strategy(crate::APP_STRATEGY.clone())
            .map(|strategy| strategy.in_config_dir(".jugglerignore"))
            .unwrap_or_else(|_| {
                PathBuf::from(shellexpand::tilde("~/.config/juggler/.jugglerignore").to_string())
            });
        ...
        let local_ignore_path = cwd.join(".jugglerignore");
        ...
        if local_ignore_path.is_file() {
            ...
            // If no .jugglerignore exists, check for .gitignore as fallback
            let gitignore_path = cwd.join(".gitignore");
            if gitignore_path.is_file() {
                tracing::debug!(
                    "No .jugglerignore found, using .gitignore as fallback for ignore patterns"
                );
        ...
        // Only use default patterns if no .jugglerignore files were found
        ...
        for arg in &cmd_parts[1..] {
            ...
            if self.is_ignored(path) {
                return Err(ToolError::ExecutionError(format!(
                    "The command attempts to access '{}' which is restricted by .jugglerignore",
                    arg
                )));
            }
        }
    ...
    async fn text_editor(&self, params: Value) -> Result<Vec<Content>, ToolError> {
        ...
        if self.is_ignored(&path) {
            return Err(ToolError::ExecutionError(format!(
                "Access to '{}' is restricted by .jugglerignore",
                path.display()
            )));
        }
...
mod tests {
    ...
    #[test]
    #[serial]
    fn test_global_jugglerhints() {
        // if ~/.config/juggler/.jugglerhints exists, it should be included in the instructions
        // copy the existing global hints file to a .bak file
        let global_hints_path =
            PathBuf::from(shellexpand::tilde("~/.config/juggler/.jugglerhints").to_string());
        let global_hints_bak_path =
            PathBuf::from(shellexpand::tilde("~/.config/juggler/.jugglerhints.bak").to_string());
        ...
        fs::write(&global_hints_path, "These are my global juggler hints.").unwrap();
        ...
        assert!(instructions.contains("my global juggler hints."));
    ...
    #[test]
    #[serial]
    fn test_jugglerhints_when_present() {
        let dir = TempDir::new().unwrap();
        std::env::set_current_dir(dir.path()).unwrap();

        fs::write(".jugglerhints", "Test hint content").unwrap();
        let router = DeveloperRouter::new();
        let instructions = router.instructions();
    ...
    #[test]
    #[serial]
    fn test_jugglerhints_when_missing() {
        let dir = TempDir::new().unwrap();
        std::env::set_current_dir(dir.path()).unwrap();
    ...
    // Test JugglerIgnore pattern matching
    #[tokio::test]
    #[serial]
    async fn test_juggler_ignore_basic_patterns() {
        let temp_dir = tempfile::tempdir().unwrap();
        std::env::set_current_dir(&temp_dir).unwrap();
    ...
    #[tokio::test]
    #[serial]
    async fn test_gitignore_fallback_when_no_jugglerignore() {
        let temp_dir = tempfile::tempdir().unwrap();
        std::env::set_current_dir(&temp_dir).unwrap();

        // Create a .gitignore file but no .jugglerignore
    ...
    #[tokio::test]
    #[serial]
    async fn test_jugglerignore_takes_precedence_over_gitignore() {
        let temp_dir = tempfile::tempdir().unwrap();
        std::env::set_current_dir(&temp_dir).unwrap();

        // Create both .jugglerignore and .gitignore files with different patterns
        std::fs::write(temp_dir.path().join(".jugglerignore"), "*.secret").unwrap();
        std::fs::write(temp_dir.path().join(".gitignore"), "*.log\ntarget/").unwrap();

        let router = DeveloperRouter::new();

        // .jugglerignore patterns should be used
        assert!(
            router.is_ignored(Path::new("test.secret")),
            "*.secret pattern from .jugglerignore should be ignored"
        );

        // .gitignore patterns should NOT be used when .jugglerignore exists
        assert!(
            !router.is_ignored(Path::new("test.log")),
            "*.log pattern from .gitignore should NOT be ignored when .jugglerignore exists"
        );
        assert!(
            !router.is_ignored(Path::new("build.tmp")),
            "*.tmp pattern from .gitignore should NOT be ignored when .jugglerignore exists"
        );
