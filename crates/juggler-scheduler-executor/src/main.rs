...
async fn main() -> Result<()> {
    ...
    info!("Starting juggler-scheduler-executor for job: {}", args.job_id);
...
async fn execute_recipe(job_id: &str, recipe_path: &str) -> Result<String> {
    ...
    let provider_name: String = global_config.get_param("JUGGLER_PROVIDER").map_err(|_| {
        anyhow!("JUGGLER_PROVIDER not configured. Run 'juggler configure' or set env var.")
    })?;
    let model_name: String = global_config.get_param("JUGGLER_MODEL").map_err(|_| {
        anyhow!("JUGGLER_MODEL not configured. Run 'juggler configure' or set env var.")
    })?;

    let model_config = juggler::model::ModelConfig::new(model_name);
    ...
    let Some(prompt_text) = recipe.prompt else {
        ...
        let session_file_path = juggler::session::storage::get_path(
            juggler::session::storage::Identifier::Name(session_id.clone()),
        );

        let metadata = juggler::session::storage::SessionMetadata {
        ...
        juggler::session::storage::save_messages_with_metadata(&session_file_path, &metadata, &[])
    ...
    let session_config = SessionConfig {
        id: juggler::session::storage::Identifier::Name(session_id.clone()),
    ...
    match juggler::session::storage::read_metadata(&session_file_path) {
        Ok(mut updated_metadata) => {
            updated_metadata.message_count = messages.len();
            juggler::session::storage::save_messages_with_metadata(
                &session_file_path,
                &updated_metadata,
                &messages,
            )
        ...
        Err(_) => {
            let fallback_metadata = juggler::session::storage::SessionMetadata {
