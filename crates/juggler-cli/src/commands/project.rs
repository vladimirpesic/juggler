...
pub fn handle_project_default() -> Result<()> {
    ...
    if projects.is_empty() {
        ...
        let mut command = std::process::Command::new("juggler");
        command.arg("session");
        let status = command.status()?;

        if !status.success() {
            println!("Failed to run Juggler. Exit code: {:?}", status.code());
        }
        return Ok(());
    }
    ...
    let _ = intro("Juggler Project Manager");
    ...
    match choice {
        "resume" => {
            ...
            // Build the command to run Juggler
            let mut command = std::process::Command::new("juggler");
            command.arg("session");
        ...
        "fresh" => {
            ...
            // Build the command to run Juggler with a fresh session
            let mut command = std::process::Command::new("juggler");
            command.arg("session");
