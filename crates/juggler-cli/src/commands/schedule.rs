...
use juggler::scheduler::{
    get_default_scheduled_recipes_dir, get_default_scheduler_storage_path, ScheduledJob,
    SchedulerError,
};
use juggler::scheduler_factory::SchedulerFactory;
use juggler::temporal_scheduler::TemporalScheduler;
use std::path::Path;
    ...
    match scheduler.add_scheduled_job(job).await {
        Ok(_) => {
            ...
            let scheduled_recipes_dir = get_default_scheduled_recipes_dir()
                .unwrap_or_else(|_| Path::new("./.juggler_scheduled_recipes").to_path_buf()); // Fallback for display
...
pub async fn handle_schedule_services_status() -> Result<()> {
    ...
    let scheduler_type =
        std::env::var("JUGGLER_SCHEDULER_TYPE").unwrap_or_else(|_| "temporal".to_string());

    if scheduler_type != "temporal" {
        println!("Service management is only available for temporal scheduler.");
        println!("Set JUGGLER_SCHEDULER_TYPE=temporal to use Temporal services.");
        return Ok(());
    }
    ...
    match TemporalScheduler::new().await {
        ...
        Err(e) => {
            ...
            println!("   2. Or use legacy scheduler: export JUGGLER_SCHEDULER_TYPE=legacy");
        }
    }
...
pub async fn handle_schedule_cron_help() -> Result<()> {
    println!("📅 Cron Expression Guide for Juggler Scheduler");
    println!("===========================================\\n");
    ...
    println!("💡 EXAMPLES:");
    println!(
        "  juggler schedule add --id hourly-report --cron \"0 * * * *\" --recipe-source report.yaml"
    );
    println!(
        "  juggler schedule add --id daily-backup --cron \"@daily\" --recipe-source backup.yaml"
    );
    println!("  juggler schedule add --id weekly-summary --cron \"0 9 * * 1\" --recipe-source summary.yaml");
