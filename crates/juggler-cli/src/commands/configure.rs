...
use console::style;
use juggler::agents::extension::ToolInfo;
use juggler::agents::extension_manager::get_parameter_names;
use juggler::agents::platform_tools::{
    PLATFORM_LIST_RESOURCES_TOOL_NAME, PLATFORM_READ_RESOURCE_TOOL_NAME,
};
use juggler::agents::Agent;
use juggler::agents::{extension::Envs, ExtensionConfig};
use juggler::config::extensions::name_to_key;
use juggler::config::permission::PermissionLevel;
use juggler::config::{
    Config, ConfigError, ExperimentManager, ExtensionConfigManager, ExtensionEntry,
    PermissionManager,
};
use juggler::message::Message;
use juggler::providers::{create, providers};
...
use crate::recipes::github_recipe::JUGGLER_RECIPE_GITHUB_REPO_CONFIG_KEY;
...
pub async fn handle_configure() -> Result<(), Box<dyn Error>> {
    ...
    if !config.exists() {
        ...
        println!(
            "{}",
            style("Welcome to juggler! Let's get you set up with a provider.").dim()
        );
        println!(
            "{}",
            ...
            Ok(true) => {
                ...
                ExtensionConfigManager::set(ExtensionEntry {
                    ...
                    config: ExtensionConfig::Builtin {
                        ...
                        display_name: Some(juggler::config::DEFAULT_DISPLAY_NAME.to_string()),
                        timeout: Some(juggler::config::DEFAULT_EXTENSION_TIMEOUT),
                        bundled: Some(true),
                    },
                })?;
            }
        ...
        let action = cliclack::select("What would you like to configure?")
            ...
            .item(
                "settings",
                "Juggler Settings",
                "Set the Juggler Mode, Tool Output, Tool Permissions, Experiment, Juggler recipe github repo and more",
            )
            .interact()?;
...
pub async fn configure_provider_dialog() -> Result<bool, Box<dyn Error>> {
    ...
    let current_provider: Option<String> = config.get_param("JUGGLER_PROVIDER").ok();
    ...
    let models_res = {
        let temp_model_config = juggler::model::ModelConfig::new(provider_meta.default_model.clone());
    ...
    let model: String = match models_res {
        ...
        Ok(None) => {
            let default_model =
                std::env::var("JUGGLER_MODEL").unwrap_or(provider_meta.default_model.clone());
    ...
    let toolshim_enabled = std::env::var("JUGGLER_TOOLSHIM")
        .map(|val| val == "1" || val.to_lowercase() == "true")
        .unwrap_or(false);

    let model_config = juggler::model::ModelConfig::new(model.clone())
        .with_max_tokens(Some(50))
        .with_toolshim(toolshim_enabled)
        .with_toolshim_model(std::env::var("JUGGLER_TOOLSHIM_OLLAMA_MODEL").ok());
    ...
    let result = provider
        .complete(
            "You are an AI agent called Juggler. You use tools of connected extensions to solve problems.",
            &messages,
            &tools
        )
        .await;
    ...
    match result {
        Ok((_message, _usage)) => {
            ...
            config.set_param("JUGGLER_PROVIDER", Value::String(provider_name.to_string()))?;
            config.set_param("JUGGLER_MODEL", Value::String(model.clone()))?;
...
/// Configure extensions that can be used with juggler
...
pub fn configure_extensions_dialog() -> Result<(), Box<dyn Error>> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with Juggler",
        )
        .item(
            "stdio",
    ...
    match extension_type {
        // TODO we'll want a place to collect all these options, maybe just an enum in juggler-mcp
        "built-in" => {
            ...
            let timeout: u64 = cliclack::input("Please set the timeout for this tool (in secs):")
                .placeholder(&juggler::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
pub async fn configure_settings_dialog() -> Result<(), Box<dyn Error>> {
    let setting_type = cliclack::select("What setting would you like to configure?")
        .item("juggler_mode", "Juggler Mode", "Configure Juggler mode")
        .item(
            "juggler_router_strategy",
        ...
        .item(
            "recipe",
            "Juggler recipe github repo",
            "Juggler will pull recipes from this repo if not found locally.",
        )
        .interact()?;

    match setting_type {
        "juggler_mode" => {
            configure_juggler_mode_dialog()?;
        }
        "juggler_router_strategy" => {
            configure_juggler_router_strategy_dialog()?;
        }
        "tool_permission" => {
...
pub fn configure_juggler_mode_dialog() -> Result<(), Box<dyn Error>> {
    let config = Config::global();

    // Check if JUGGLER_MODE is set as an environment variable
    if std::env::var("JUGGLER_MODE").is_ok() {
        let _ = cliclack::log::info("Notice: JUGGLER_MODE environment variable is set and will override the configuration here.");
    }

    let mode = cliclack::select("Which Juggler mode would you like to configure?")
        .item(
            "auto",
            "Auto Mode",
    ...
    match mode {
        "auto" => {
            config.set_param("JUGGLER_MODE", Value::String("auto".to_string()))?;
            cliclack::outro("Set to Auto Mode - full file modification enabled")?;
        }
        "approve" => {
            config.set_param("JUGGLER_MODE", Value::String("approve".to_string()))?;
            cliclack::outro("Set to Approve Mode - all tools and modifications require approval")?;
        }
        "smart_approve" => {
            config.set_param("JUGGLER_MODE", Value::String("smart_approve".to_string()))?;
            cliclack::outro("Set to Smart Approve Mode - modifications require approval")?;
        }
        "chat" => {
            config.set_param("JUGGLER_MODE", Value::String("chat".to_string()))?;
...
pub fn configure_juggler_router_strategy_dialog() -> Result<(), Box<dyn Error>> {
    let config = Config::global();

    // Check if JUGGLER_ROUTER_STRATEGY is set as an environment variable
    if std::env::var("JUGGLER_ROUTER_TOOL_SELECTION_STRATEGY").is_ok() {
        let _ = cliclack::log::info("Notice: JUGGLER_ROUTER_TOOL_SELECTION_STRATEGY environment variable is set. Configuration will override this.");
    }
    ...
    match strategy {
        "vector" => {
            config.set_param(
                "JUGGLER_ROUTER_TOOL_SELECTION_STRATEGY",
...
pub fn configure_tool_output_dialog() -> Result<(), Box<dyn Error>> {
    let config = Config::global();
    // Check if JUGGLER_CLI_MIN_PRIORITY is set as an environment variable
    if std::env::var("JUGGLER_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info("Notice: JUGGLER_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.");
    }
    ...
    match tool_log_level {
        "high" => {
            config.set_param("JUGGLER_CLI_MIN_PRIORITY", Value::from(0.8))?;
            cliclack::outro("Showing tool output of high importance only.")?;
        }
        "medium" => {
            config.set_param("JUGGLER_CLI_MIN_PRIORITY", Value::from(0.2))?;
            cliclack::outro("Showing tool output of medium importance.")?;
        }
        "all" => {
            config.set_param("JUGGLER_CLI_MIN_PRIORITY", Value::from(0.0))?;
...
/// Configure experiment features that can be used with juggler
...
fn configure_recipe_dialog() -> Result<(), Box<dyn Error>> {
    let key_name = JUGGLER_RECIPE_GITHUB_REPO_CONFIG_KEY;
    let config = Config::global();
    ...
    let mut recipe_repo_input = cliclack::input(
        "Enter your Juggler Recipe Github repo (owner/repo): eg: my_org/juggler-recipes",
    )
    .required(false);
