    ...
    match load_recipe(recipe_name) {
        Ok(recipe) => {
            ...
            if let Ok(recipe_json) = serde_json::to_string(&recipe) {
                ...
                full_url = format!("juggler://recipe?config={}", url_safe);
                println!("{}", full_url);
            }
            Ok(full_url)
        }
...
"#;
    ...
    fn test_handle_deeplink_valid_recipe() {
        ...
        assert!(result.is_ok());
        assert!(result.unwrap().contains("juggler://recipe?config=eyJ2ZXJzaW9uIjoiMS4wLjAiLCJ0aXRsZSI6IlRlc3QgUmVjaXBlIiwiZGVzY3JpcHRpb24iOiJBIHRlc3QgcmVjaXBlIGZvciBkZWVwbGluayBnZW5lcmF0aW9uIiwiaW5zdHJ1Y3Rpb25zIjoiVGVzdCBpbnN0cnVjdGlvbnMiLCJwcm9tcHQiOiJUZXN0IHByb21wdCBjb250ZW50In0%3D"));
    }
