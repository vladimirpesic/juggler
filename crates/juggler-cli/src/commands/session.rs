...
use juggler::session::info::{get_session_info, SessionInfo, SortOrder};
use juggler::session::{self, Identifier};
use regex::Regex;
use std::fs;
...
pub fn handle_session_export(identifier: Identifier, output_path: Option<PathBuf>) -> Result<()> {
    ...
    let session_file_path = match juggler::session::get_path(identifier.clone()) {
        Ok(path) => path,
        Err(e) => {
    ...
    let messages = match juggler::session::read_messages(&session_file_path) {
        Ok(msgs) => msgs,
        Err(e) => {
...
fn export_session_to_markdown(
    messages: Vec<juggler::message::Message>,
    session_file: &Path,
    ...
    for message in &messages {
        ...
        let is_only_tool_response = message.role == mcp_core::role::Role::User
            && message
                .content
                .iter()
                .all(|content| matches!(content, juggler::message::MessageContent::ToolResponse(_)));
        ...
        if message
            .content
            .iter()
            .any(|content| matches!(content, juggler::message::MessageContent::ToolRequest(_)))
        {
...
pub fn prompt_interactive_session_selection() -> Result<session::Identifier> {
    ...
    if let Some(session) = display_map.get(&selected_display_text) {
        Ok(juggler::session::Identifier::Name(session.id.clone()))
    } else {
