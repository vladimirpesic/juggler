...
use juggler::agents::{Agent, AgentEvent};
use juggler::message::Message as JugglerMessage;
use juggler::session;
...
type SessionStore = Arc<RwLock<std::collections::HashMap<String, Arc<Mutex<Vec<JugglerMessage>>>>>>;
...
pub async fn handle_web(port: u16, host: String, open: bool) -> Result<()> {
    // Setup logging
    crate::logging::setup_logging(Some("juggler-web"), None)?;

    // Load config and create agent just like the CLI does
    let config = juggler::config::Config::global();

    let provider_name: String = match config.get_param("JUGGLER_PROVIDER") {
        Ok(p) => p,
        Err(_) => {
            eprintln!("No provider configured. Run 'juggler configure' first");
            std::process::exit(1);
        }
    };

    let model: String = match config.get_param("JUGGLER_MODEL") {
        Ok(m) => m,
        Err(_) => {
            eprintln!("No model configured. Run 'juggler configure' first");
            std::process::exit(1);
        }
    };
    ...
    // Create the agent
    let agent = Agent::new();
    let provider = juggler::providers::create(&provider_name, model_config)?;
    agent.update_provider(provider).await?;

    // Load and enable extensions from config
    let extensions = juggler::config::ExtensionConfigManager::get_all()?;
...
async fn serve_session(
    ...
    let html_with_session = html.replace(
        ...
        &format!(
            "<script>window.JUGGLER_SESSION_NAME = '{}';</script>\n    <script src=\"/static/script.js\"></script>",
            session_name
        )
    );
...
async fn process_message_streaming(
    agent: &Agent,
    session_messages: Arc<Mutex<Vec<JugglerMessage>>>,
    session_file: std::path::PathBuf,
    content: String,
    sender: Arc<Mutex<futures::stream::SplitSink<WebSocket, Message>>>,
) -> Result<()> {
    use futures::StreamExt;
    use juggler::agents::SessionConfig;
    use juggler::message::MessageContent;
    use juggler::session;

    // Create a user message
    let user_message = JugglerMessage::user().with_text(content.clone());
    ...
    if provider.is_err() {
        let error_msg = "I'm not properly configured yet. Please configure a provider through the CLI first using `juggler configure`.".to_string();
        let mut sender = sender.lock().await;
        let _ = sender
            .send(Message::Text(
                            ...
                            match content {
                                ...
                                MessageContent::ToolConfirmationRequest(confirmation) => {
                                    ...
                                    agent.handle_confirmation(
                                        ...
                                        juggler::permission::PermissionConfirmation {
                                            principal_type: juggler::permission::permission_confirmation::PrincipalType::Tool,
                                            permission: juggler::permission::Permission::AllowOnce,
                                        }
                                    ).await;
                                }
