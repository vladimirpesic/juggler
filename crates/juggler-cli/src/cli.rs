...
use juggler::config::{Config, ExtensionConfig};
...
use juggler_bench::bench_config::BenchRunConfig;
use juggler_bench::runners::bench_runner::BenchRunner;
use juggler_bench::runners::eval_runner::EvalRunner;
use juggler_bench::runners::metric_aggregator::MetricAggregator;
use juggler_bench::runners::model_runner::ModelRunner;
use std::io::Read;
...
#[derive(Subcommand)]
enum Command {
    /// Configure Juggler settings
    #[command(about = "Configure Juggler settings")]
    Configure {},

    /// Display Juggler configuration information
    #[command(about = "Display Juggler information")]
    Info {
    ...
    #[command(about = "Run one of the mcp servers bundled with juggler")]
    Mcp { name: String },
    ...
    Session {
        ...
        #[arg(
            ...
            long_help = "Add one or more builtin extensions that are bundled with juggler by specifying their names, comma-separated",
            value_delimiter = ','
        )]
        builtins: Vec<String>,
    },
    ...
    Run {
        ...
        #[arg(
            ...
            long = "text",
            value_name = "TEXT",
            help = "Input text to provide to Juggler directly",
            long_help = "Input text containing commands for Juggler. Use this in lieu of the instructions argument.",
        ...
        #[arg(
            ...
            help = "Dynamic parameters (e.g., --params username=alice --params channel_name=juggler-channel)",
    ...
    /// Update the Juggler CLI version
    #[command(about = "Update the juggler CLI version")]
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the juggler CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,

        /// Enforce to re-configure Juggler during update
        #[arg(short, long, help = "Enforce to re-configure juggler during update")]
        reconfigure: bool,
    },
            ...
            let (input_config, session_settings) = match (instructions, input_text, recipe, explain)
            {
                ...
                (Some(file), _, _, _) => {
                    let contents = std::fs::read_to_string(&file).unwrap_or_else(|err| {
                        eprintln!(
                            "Instruction file not found — did you mean to use juggler run --text?\n{}",
                            err
                        );
                        std::process::exit(1);
                    });
                ...
                (_, _, Some(recipe_name), explain) => {
                    ...
                    (
                        ...
                        recipe.settings.map(|s| SessionSettings {
                            juggler_provider: s.juggler_provider,
                            juggler_model: s.juggler_model,
