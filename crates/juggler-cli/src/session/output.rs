...
use juggler::message::{Message, MessageContent, ToolRequest, ToolResponse};
...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("JUGGLER_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("JUGGLER_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("JUGGLER_CLI_THEME", Value::String(theme.as_config_string()))
    ...
    for content in &message.content {
        match content {
            ...
            MessageContent::Thinking(thinking) => {
                if std::env::var("JUGGLER_CLI_SHOW_THINKING").is_ok() {
...
pub fn juggler_mode_message(text: &str) {
    println!("\n{}", style(text).yellow(),);
}
        ...
        Ok(contents) => {
            for content in contents {
                ...
                let min_priority = config
                    .get_param::<f32>("JUGGLER_CLI_MIN_PRIORITY")
                    .ok()
                    .unwrap_or(0.5);
...
fn get_tool_params_max_length() -> usize {
    Config::global()
        .get_param::<usize>("JUGGLER_CLI_TOOL_PARAMS_TRUNCATION_MAX_LENGTH")
        .ok()
        .unwrap_or(40)
}
...
pub fn display_session_info(
    ...
    model: &str,
    session_file: &Path,
    provider_instance: Option<&Arc<dyn juggler::providers::base::Provider>>,
) {
...
pub fn display_greeting() {
    println!("\nJuggler is running! Enter your instructions, or try asking what juggler can do.\n");
}
