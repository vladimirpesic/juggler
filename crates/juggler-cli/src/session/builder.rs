use console::style;
use juggler::agents::extension::ExtensionError;
use juggler::agents::Agent;
use juggler::config::{Config, ExtensionConfig, ExtensionConfigManager};
use juggler::providers::create;
use juggler::session;
use juggler::session::Identifier;
...
/// Configuration for building a new Juggler session
///
...
pub struct SessionBuilderConfig {
    ...
    /// Settings to override the global Juggler settings
...
async fn offer_extension_debugging_help(
    extension_name: &str,
    error_message: &str,
    provider: Arc<dyn juggler::providers::base::Provider>,
    interactive: bool,
) -> Result<(), anyhow::Error> {
    ...
    let temp_session_file =
        std::env::temp_dir().join(format!("juggler_debug_extension_{}.jsonl", extension_name));
...
pub struct SessionSettings {
    pub juggler_model: Option<String>,
    pub juggler_provider: Option<String>,
...
pub async fn build_session(session_config: SessionBuilderConfig) -> Session {
    ...
    let provider_name = session_config
        .settings
        .as_ref()
        .and_then(|s| s.juggler_provider.clone())
        .or_else(|| config.get_param("JUGGLER_PROVIDER").ok())
        .expect("No provider configured. Run 'juggler configure' first");

    let model_name = session_config
        .settings
        .as_ref()
        .and_then(|s| s.juggler_model.clone())
        .or_else(|| config.get_param("JUGGLER_MODEL").ok())
    ...
    let model_config =
        juggler::model::ModelConfig::new(model_name.clone()).with_temperature(temperature);
    ...
    let new_provider = match create(&provider_name, model_config) {
        ...
        Err(e) => {
            output::render_error(&format!(
                "Error {}.\n\
                Please check your system keychain and run 'juggler configure' again.\n\
                If your system is unable to use the keyring, please try setting secret key(s) via environment variables.\n\
                For more info, see: https://block.github.io/juggler/docs/troubleshooting/#keychainkeyring-errors",
                e
            ));
            process::exit(1);
        }
    ...
    let system_prompt_file: Option<String> = config.get_param("JUGGLER_SYSTEM_PROMPT_FILE_PATH").ok();
