use super::completion::JugglerCompleter;
use anyhow::Result;
...
pub fn get_input(
    editor: &mut Editor<JugglerCompleter, rustyline::history::DefaultHistory>,
) -> Result<InputResult> {
...
fn handle_slash_command(input: &str) -> Option<InputResult> {
    ...
    match input {
        ...
        s if s.starts_with(CMD_MODE) => {
            Some(InputResult::JugglerMode(s[CMD_MODE.len()..].to_string()))
        }
...
/mode <name> - Set the juggler mode to use ('auto', 'approve', 'chat')
/plan <message_text> -  Enters 'plan' mode with optional message. Create a plan based on the current messages and asks user if they want to act on it.
                        If user acts on the plan, juggler mode is set to 'auto' and returns to 'normal' juggler mode.
                        To warm up juggler before using '/plan', we recommend setting '/mode approve' & putting appropriate context into juggler.
                        The model is used based on $JUGGLER_PLANNER_PROVIDER and $JUGGLER_PLANNER_MODEL environment variables.
                        If no model is set, the default model is used.
/endplan - Exit plan mode and return to 'normal' juggler mode.
...
Navigation:
Ctrl+C - Interrupt juggler (resets the interaction to before the interrupted user request)
