...
use console::Color;
use juggler::agents::AgentEvent;
use juggler::permission::permission_confirmation::PrincipalType;
use juggler::permission::Permission;
use juggler::permission::PermissionConfirmation;
use juggler::providers::base::Provider;
pub use juggler::session::Identifier;

use anyhow::{Context, Result};
use completion::JugglerCompleter;
use etcetera::{choose_app_strategy, AppStrategy};
use juggler::agents::extension::{Envs, ExtensionConfig};
use juggler::agents::{Agent, SessionConfig};
use juggler::config::Config;
use juggler::message::{Message, MessageContent};
use juggler::session;
...
impl Session {
    ...
    pub async fn add_extension(&mut self, extension_command: String) -> Result<()> {
        ...
        let config = ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: Some(juggler::config::DEFAULT_EXTENSION_DESCRIPTION.to_string()),
    ...
    pub async fn interactive(&mut self, message: Option<String>) -> Result<()> {
        ...
        let config = rustyline::Config::builder()
            ...
            .build();
        let mut editor =
            rustyline::Editor::<JugglerCompleter, rustyline::history::DefaultHistory>::with_config(
                config,
            )?;

        // Set up the completer with a reference to the completion cache
        let completer = JugglerCompleter::new(self.completion_cache.clone());
        editor.set_helper(Some(completer));

        // Create and use a global history file in ~/.config/juggler directory
        ...
        let strategy =
            choose_app_strategy(crate::APP_STRATEGY.clone()).expect("juggler requires a home dir");
        ...
        let save_history =
            |editor: &mut rustyline::Editor<JugglerCompleter, rustyline::history::DefaultHistory>| {
        ...
        loop {
            ...
            match input::get_input(&mut editor)? {
                ...
                input::InputResult::JugglerMode(mode) => {
                    save_history(&mut editor);
                    ...
                    config
                        .set_param("JUGGLER_MODE", Value::String(mode.to_string()))
                        .unwrap();
                    output::juggler_mode_message(&format!("Juggler mode set to '{}'", mode));
                    continue;
                }
        ...
        match planner_response_type {
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    self.run_mode = RunMode::Normal;
                    // set juggler mode: auto if that isn't already the case
                    let config = Config::global();
                    let curr_juggler_mode =
                        config.get_param("JUGGLER_MODE").unwrap_or("auto".to_string());
                    if curr_juggler_mode != "auto" {
                        config
                            .set_param("JUGGLER_MODE", Value::String("auto".to_string()))
                            .unwrap();
                    }
                    ...
                    // Reset run & juggler mode
                    if curr_juggler_mode != "auto" {
                        config
                            .set_param("JUGGLER_MODE", Value::String(curr_juggler_mode.to_string()))
                            .unwrap();
                    }
                } else {
                    ...
                    match result {
                        Some(Ok(AgentEvent::Message(message))) => {
                            ...
                            if let Some(MessageContent::ToolConfirmationRequest(confirmation)) = message.content.first() {
                                ...
                                let prompt = "Juggler would like to call the above tool, do you allow?".to_string();
                                ...
                                let config = Config::global();
                                let context_strategy = config.get_param::<String>("JUGGLER_CONTEXT_STRATEGY")
                                    ...
                                    _ => {
                                        if interactive {
                                            ...
                                            cliclack::select(prompt)
                                                .item("clear", "Clear Session", "Removes all messages from Juggler's memory")
                                ...
                                match selected {
                                    ...
                                    "truncate" => {
                                        ...
                                        let msg = if context_strategy == "truncate" {
                                            format!("Context maxed out - automatically truncated messages.\n{}\nJuggler tried its best to truncate messages for you.", "-".repeat(50))
                                        } else {
                                    ...
                                    "summarize" => {
                                        ...
                                        let message_suffix = if context_strategy == "summarize" {
                                            "Juggler automatically summarized messages for you."
                                        } else if interactive {
                                            "Juggler summarized messages for you."
                                        } else {
                                            "Juggler automatically summarized messages to continue processing."
                                        };
...
fn get_reasoner() -> Result<Arc<dyn Provider>, anyhow::Error> {
    use juggler::model::ModelConfig;
    use juggler::providers::create;

    let config = Config::global();

    // Try planner-specific provider first, fallback to default provider
    let provider = if let Ok(provider) = config.get_param::<String>("JUGGLER_PLANNER_PROVIDER") {
        provider
    } else {
        println!("WARNING: JUGGLER_PLANNER_PROVIDER not found. Using default provider...");
        config
            .get_param::<String>("JUGGLER_PROVIDER")
            .expect("No provider configured. Run 'juggler configure' first")
    };

    // Try planner-specific model first, fallback to default model
    let model = if let Ok(model) = config.get_param::<String>("JUGGLER_PLANNER_MODEL") {
        model
    } else {
        println!("WARNING: JUGGLER_PLANNER_MODEL not found. Using default model...");
        config
            .get_param::<String>("JUGGLER_MODEL")
            .expect("No model configured. Run 'juggler configure' first")
    };
