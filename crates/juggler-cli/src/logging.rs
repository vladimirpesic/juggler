...
use juggler::tracing::langfuse_layer;
use juggler_bench::bench_session::BenchAgentError;
use juggler_bench::error_capture::ErrorCaptureLayer;
...
fn get_log_directory_with_date(test_date: Option<String>) -> Result<PathBuf> {
    ...
    // - macOS/Linux: ~/.local/state/juggler/logs/cli
    // - Windows:     ~\AppData\Roaming\juggler\data\logs\cli
...
fn setup_logging_internal(
    ...
    let mut setup = || {
        result = (|| {
            // Set up file appender for juggler module logs
            ...
            let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                ...
                EnvFilter::new("")
                    ...
                    // Set juggler module to DEBUG
                    .add_directive("juggler=debug".parse().unwrap())
                    // Set juggler-cli to INFO
                    .add_directive("juggler_cli=info".parse().unwrap())
...
mod tests {
    ...
    fn test_log_directory_creation() {
        ...
        assert!(path_components.iter().any(|c| c.as_os_str() == "juggler"));
