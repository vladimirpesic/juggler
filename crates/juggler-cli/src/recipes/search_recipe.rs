...
use super::github_recipe::{retrieve_recipe_from_github, JUGGLER_RECIPE_GITHUB_REPO_CONFIG_KEY};

const JUGGLER_RECIPE_PATH_ENV_VAR: &str = "JUGGLER_RECIPE_PATH";
...
fn retrieve_recipe_from_local_path(recipe_name: &str) -> Result<(String, PathBuf)> {
    ...
    if let Ok(recipe_path_env) = env::var(JUGGLER_RECIPE_PATH_ENV_VAR) {
...
fn configured_github_recipe_repo() -> Option<String> {
    ...
    match config.get_param(JUGGLER_RECIPE_GITHUB_REPO_CONFIG_KEY) {
