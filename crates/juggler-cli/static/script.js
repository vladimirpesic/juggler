...
function getSessionId() {
    ...
    if (window.JUGGLER_SESSION_NAME) {
        return window.JUGGLER_SESSION_NAME;
    }
...
function addThinkingIndicator() {
    ...
    thinkingDiv.innerHTML = `
        ...
        <span class="thinking-text">Juggler is thinking...</span>
    `;
        ...
        if (response.ok) {
            ...
            if (sessionData.messages && sessionData.messages.length > 0) {
                ...
                if (sessionData.metadata && sessionData.metadata.description) {
                    document.title = `Juggler Chat - ${sessionData.metadata.description}`;
                }
...
function updateSessionTitle() {
    ...
    // Just show "Juggler Chat" - no need to show session ID
    titleElement.textContent = 'Juggler Chat';
}
