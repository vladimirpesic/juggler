...
impl EvalRunner {
    ...
    fn create_work_dir(&self, config: &BenchRunConfig) -> Result<BenchmarkWorkDir> {
        let juggler_model = config
            .models
            .first()
            .context("No model specified in configuration")?;
        let model_name = juggler_model.name.clone();
        let provider_name = juggler_model.provider.clone();
        ...
        let work_dir_name_shim = {
            ...
            if let Some(shim_opt) = &juggler_model.tool_shim {
