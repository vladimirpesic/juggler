...
impl ModelRunner {
    ...
    fn run_benchmark(
        ...
        envs.push(("JUGGLER_MODEL".to_string(), model.clone().name));
        envs.push(("JUGGLER_PROVIDER".to_string(), model.clone().provider));
        ...
        if let Some(model) = self.config.models.first() {
            if let Some(shim_opt) = &model.tool_shim {
                if shim_opt.use_tool_shim {
                    shim_envs.push(("JUGGLER_TOOLSHIM".to_string(), "true".to_string()));
                    if let Some(shim_model) = &shim_opt.tool_shim_model {
                        shim_envs.push((
                            "JUGGLER_TOOLSHIM_OLLAMA_MODEL".to_string(),
                            shim_model.clone(),
                        ));
                    }
                }
