...
impl SchedulerType {
    pub fn from_config() -> Self {
        ...
        // If not, always use legacy scheduler regardless of JUGGLER_SCHEDULER_TYPE
        ...
        match config.get_param::<String>("JUGGLER_SCHEDULER_TYPE") {
...
mod tests {
    ...
    fn test_scheduler_type_no_alpha_env() {
        ...
        with_vars(
            [
                ("ALPHA", None::<&str>),
                ("JUGGLER_SCHEDULER_TYPE", Some("temporal")),
            ],
            || {
                let scheduler_type = SchedulerType::from_config();
    ...
    fn test_scheduler_type_alpha_true_legacy() {
        // Test that with ALPHA=true and JUGGLER_SCHEDULER_TYPE=legacy, we get Legacy scheduler
        with_vars(
            [
                ("ALPHA", Some("true")),
                ("JUGGLER_SCHEDULER_TYPE", Some("legacy")),
            ],
            || {
                let scheduler_type = SchedulerType::from_config();
                assert!(matches!(scheduler_type, SchedulerType::Legacy));
    ...
    fn test_scheduler_type_alpha_true_unknown_scheduler_type() {
        ...
        with_vars(
            [
                ("ALPHA", Some("true")),
                ("JUGGLER_SCHEDULER_TYPE", Some("unknown")),
            ],
            || {
                let scheduler_type = SchedulerType::from_config();
                assert!(matches!(scheduler_type, SchedulerType::Legacy));
