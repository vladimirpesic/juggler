...
use crate::providers::base::Provider as JugglerProvider; // Alias to avoid conflict in test section
...
async fn run_scheduled_job_internal(
    job: ScheduledJob,
    provider_override: Option<Arc<dyn JugglerProvider>>, // New optional parameter
    ...
    let agent_provider: Arc<dyn JugglerProvider>; // Use the aliased JugglerProvider

    if let Some(provider) = provider_override {
        agent_provider = provider;
    } else {
        let global_config = Config::global();
        let provider_name: String = match global_config.get_param("JUGGLER_PROVIDER") {
            Ok(name) => name,
            Err(_) => return Err(JobExecutionError {
                job_id: job.id.clone(),
                error:
                    "JUGGLER_PROVIDER not configured globally. Run 'juggler configure' or set env var."
                        .to_string(),
            }),
        };
        let model_name: String =
            match global_config.get_param("JUGGLER_MODEL") {
                Ok(name) => name,
                Err(_) => return Err(JobExecutionError {
                    job_id: job.id.clone(),
                    error:
                        "JUGGLER_MODEL not configured globally. Run 'juggler configure' or set env var."
                            .to_string(),
                }),
            };
...
mod tests {
    ...
    impl JugglerProvider for MockSchedulerTestProvider {
    ...
    async fn test_scheduled_session_has_schedule_id() -> Result<(), Box<dyn std::error::Error>> {
        ...
        env::set_var("JUGGLER_PROVIDER", "test_provider");
        env::set_var("JUGGLER_MODEL", "test_model");
        ...
        env::remove_var("JUGGLER_PROVIDER");
        env::remove_var("JUGGLER_MODEL");
