...
impl Agent {
    ...
    pub async fn reply(
        ...
        // Get juggler_mode from config, but override with execution_mode if provided in session config
        let mut juggler_mode = config.get_param("GOOSE_MODE").unwrap_or("auto".to_string());

        // If this is a scheduled job with an execution_mode, override the juggler_mode
        if let Some(session_config) = &session {
            if let Some(execution_mode) = &session_config.execution_mode {
                ...
                juggler_mode = match execution_mode.as_str() {
                    "foreground" => "auto".to_string(),
                    "background" => "chat".to_string(),
                    _ => juggler_mode,
                };
                tracing::info!(
                    "Using execution_mode '{}' which maps to juggler_mode '{}'",
                    execution_mode,
                    juggler_mode
                );
            }
            ...
            loop {
                ...
                match Self::generate_response_from_provider(
                    ...
                    Ok((response, usage)) => {
                        ...
                        // Process tool requests depending on frontend tools and then juggler_mode
                        ...
                        // Clone juggler_mode once before the match to avoid move issues
                        let mode = juggler_mode.clone();
                        if mode.as_str() == "chat" {
                            ...
                            // At this point, we have handled the frontend tool requests and know juggler_mode != "chat"
                            // What remains is handling the remaining tool requests (enable extension,
                            // regular tool calls) in juggler_mode == ["auto", "approve" or "smart_approve"]
    ...
    pub async fn create_recipe(&self, mut messages: Vec<Message>) -> Result<Recipe> {
        ...
        let settings = Settings {
            juggler_provider: Some(provider_name.clone()),
            juggler_model: Some(model_name.clone()),
