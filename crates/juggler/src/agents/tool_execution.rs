...
pub const CHAT_MODE_TOOL_SKIPPED_RESPONSE: &str = "Let the user know the tool call was skipped in Juggler chat mode. \
            ...
            for request in tool_requests {
                if let Ok(tool_call) = request.tool_call.clone() {
                    let confirmation = Message::user().with_tool_confirmation_request(
                        ...
                        Some("Juggler would like to call the above tool. Allow? (y/n):".to_string()),
                    );
                    yield confirmation;
