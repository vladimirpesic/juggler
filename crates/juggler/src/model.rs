...
impl ModelConfig {
    ...
    pub fn new(model_name: String) -> Self {
        ...
        let toolshim = std::env::var("JUGGLER_TOOLSHIM")
            .map(|val| val == "1" || val.to_lowercase() == "true")
            .unwrap_or(false);

        let toolshim_model = std::env::var("JUGGLER_TOOLSHIM_OLLAMA_MODEL").ok();

        let temperature = std::env::var("JUGGLER_TEMPERATURE")
            .ok()
...
mod tests {
    ...
    fn test_model_config_temp_env_var() {
        ...
        with_var("JUGGLER_TEMPERATURE", Some("0.128"), || {
            let config = ModelConfig::new("test-model".to_string());
            assert_eq!(config.temperature, Some(0.128));
        });

        with_var("JUGGLER_TEMPERATURE", Some("notanum"), || {
            let config = ModelConfig::new("test-model".to_string());
            assert_eq!(config.temperature, None);
        });

        with_var("JUGGLER_TEMPERATURE", Some(""), || {
            let config = ModelConfig::new("test-model".to_string());
            assert_eq!(config.temperature, None);
        });
