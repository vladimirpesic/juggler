...
#[cfg(test)]
const TEST_KEYRING_SERVICE: &str = "juggler-test";
...
/// 2. Configuration file (~/.config/juggler/config.yaml by default)
///
/// Secrets are loaded with the following precedence:
/// 1. Environment variables (exact key match)
/// 2. System keyring (which can be disabled with JUGGLER_DISABLE_KEYRING)
/// 3. If the keyring is disabled, secrets are stored in a secrets file
///    (~/.config/juggler/secrets.yaml by default)
///
/// # Examples
///
/// ```no_run
...
///
/// For Juggler-specific configuration, consider prefixing with "juggler_" to avoid conflicts.
pub struct Config {
    config_path: PathBuf,
...
impl Default for Config {
    fn default() -> Self {
        ...
        let secrets = match env::var("JUGGLER_DISABLE_KEYRING") {
...
impl Config {
    ...
    ///
    /// This will initialize the configuration with the default path (~/.config/juggler/config.yaml)
