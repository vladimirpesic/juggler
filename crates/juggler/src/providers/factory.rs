...
pub fn create(name: &str, model: ModelConfig) -> Result<Arc<dyn Provider>> {
    ...
    if let Ok(lead_model_name) = config.get_param::<String>("JUGGLER_LEAD_MODEL") {
...
mod tests {
    ...
    fn test_create_lead_worker_provider() {
        ...
        let saved_lead = env::var("JUGGLER_LEAD_MODEL").ok();
        let saved_provider = env::var("JUGGLER_LEAD_PROVIDER").ok();
        let saved_turns = env::var("JUGGLER_LEAD_TURNS").ok();

        // Test with basic lead model configuration
        env::set_var("JUGGLER_LEAD_MODEL", "gpt-4o");
        ...
        env::set_var("JUGGLER_LEAD_PROVIDER", "anthropic");
        env::set_var("JUGGLER_LEAD_TURNS", "5");
        ...
        // Restore env vars
        match saved_lead {
            Some(val) => env::set_var("JUGGLER_LEAD_MODEL", val),
            None => env::remove_var("JUGGLER_LEAD_MODEL"),
        }
        match saved_provider {
            Some(val) => env::set_var("JUGGLER_LEAD_PROVIDER", val),
            None => env::remove_var("JUGGLER_LEAD_PROVIDER"),
        }
        match saved_turns {
            Some(val) => env::set_var("JUGGLER_LEAD_TURNS", val),
            None => env::remove_var("JUGGLER_LEAD_TURNS"),
        }
    }
    ...
    fn test_lead_model_env_vars_with_defaults() {
        // Save current env vars
        let saved_vars = [
            ("JUGGLER_LEAD_MODEL", env::var("JUGGLER_LEAD_MODEL").ok()),
            ("JUGGLER_LEAD_PROVIDER", env::var("JUGGLER_LEAD_PROVIDER").ok()),
            ("JUGGLER_LEAD_TURNS", env::var("JUGGLER_LEAD_TURNS").ok()),
            (
                "JUGGLER_LEAD_FAILURE_THRESHOLD",
                env::var("JUGGLER_LEAD_FAILURE_THRESHOLD").ok(),
            ),
            (
                "JUGGLER_LEAD_FALLBACK_TURNS",
                env::var("JUGGLER_LEAD_FALLBACK_TURNS").ok(),
            ),
        ];
        ...
        env::set_var("JUGGLER_LEAD_TURNS", "7");
        env::set_var("JUGGLER_LEAD_FAILURE_THRESHOLD", "4");
        env::set_var("JUGGLER_LEAD_FALLBACK_TURNS", "3");
    ...
    fn test_create_regular_provider_without_lead_config() {
        ...
        let saved_provider = env::var("JUGGLER_LEAD_PROVIDER").ok();
        let saved_turns = env::var("JUGGLER_LEAD_TURNS").ok();
        let saved_threshold = env::var("JUGGLER_LEAD_FAILURE_THRESHOLD").ok();
        let saved_fallback = env::var("JUGGLER_LEAD_FALLBACK_TURNS").ok();

        // Ensure all JUGGLER_LEAD_* variables are not set
        env::remove_var("JUGGLER_LEAD_MODEL");
        env::remove_var("JUGGLER_LEAD_PROVIDER");
        env::remove_var("JUGGLER_LEAD_TURNS");
        env::remove_var("JUGGLER_LEAD_FAILURE_THRESHOLD");
        env::remove_var("JUGGLER_LEAD_FALLBACK_TURNS");
        ...
        if let Some(val) = saved_lead {
            env::set_var("JUGGLER_LEAD_MODEL", val);
        }
        if let Some(val) = saved_provider {
            env::set_var("JUGGLER_LEAD_PROVIDER", val);
        }
        if let Some(val) = saved_turns {
            env::set_var("JUGGLER_LEAD_TURNS", val);
        }
        if let Some(val) = saved_threshold {
            env::set_var("JUGGLER_LEAD_FAILURE_THRESHOLD", val);
        }
        if let Some(val) = saved_fallback {
            env::set_var("JUGGLER_LEAD_FALLBACK_TURNS", val);
        }
    }
}
