...
/// Fetches and parses the allowed extensions from the URL specified in JUGGLER_ALLOWLIST env var
#[allow(dead_code)]
fn fetch_allowed_extensions() -> Option<AllowedExtensions> {
    match env::var("JUGGLER_ALLOWLIST") {
        Err(_) => {
...
fn is_command_allowed(cmd: &str, args: &[String]) -> bool {
    ...
    if let Ok(bypass_value) = env::var("JUGGLER_ALLOWLIST_BYPASS") {
        if bypass_value.to_lowercase() == "true" {
            // Bypass the allowlist check
            println!("Allowlist check bypassed due to JUGGLER_ALLOWLIST_BYPASS=true");
            return true;
        }
    }
...
fn is_command_allowed_with_allowlist(
    ...
    // Special case: Always allow commands ending with "/jugglerd" or equal to "jugglerd"
    // But still enforce that it's in the same directory as the current executable
    if cmd_base == "jugglerd" {
        ...
        if cmd == first_part {
                ...
                && !first_part.starts_with("./")
            {
                ...
                let expected_path = current_exe_dir.join("jugglerd").to_str().unwrap().to_string();
                ...
                println!("Jugglerd not in expected directory: {}", cmd);
                println!("Expected path: {}", expected_path);
                return false;
            } else {
                // For non-path jugglerd or relative paths, allow it
                return true;
            }
        }
        return false;
...
mod tests {
    ...
    fn test_normalize_command_name() {
        ...
        assert_eq!(normalize_command_name("jugglerd.exe"), "jugglerd");
        assert_eq!(
            normalize_command_name("/path/to/jugglerd.exe"),
            "/path/to/jugglerd"
        );
        ...
        assert_eq!(normalize_command_name("jugglerd"), "jugglerd");
        assert_eq!(normalize_command_name("/path/to/jugglerd"), "/path/to/jugglerd");
    }
    ...
    fn test_command_allowed_flexible() {
        ...
        // Test with shim path - 'Juggler.app/Contents/Resources/bin/' and before can be stripped to get the command to match
        assert!(is_command_allowed_with_allowlist(
            "/private/var/folders/fq/rd_cb6/T/AppTranslocation/EA0195/d/Juggler.app/Contents/Resources/bin/uvx something",
            &allowlist
        ));

        // Test with shim path & latest version
        assert!(is_command_allowed_with_allowlist(
            "/private/var/folders/fq/rd_cb6/T/AppTranslocation/EA0195/d/Juggler.app/Contents/Resources/bin/uvx something@latest",
            &allowlist
        ));
        ...
        // Test with shim path & latest version
        assert!(is_command_allowed_with_allowlist(
            "/private/var/folders/fq/rd_cb6/T/AppTranslocation/EA0195/d/Juggler.app/Contents/Resources/bin/npx -y mcp_hammer@latest start",
            &allowlist
        ));
    }
    ...
    #[test]
    fn test_jugglerd_special_case() {
        // Create a restrictive allowlist that doesn't include jugglerd
        let allowlist = create_test_allowlist(&["uvx mcp_slack"]);

        // Get the current executable directory for jugglerd path testing
        let current_exe = std::env::current_exe().unwrap();
        let current_exe_dir = current_exe.parent().unwrap();
        let jugglerd_path = current_exe_dir.join("jugglerd").to_str().unwrap().to_string();
        let jugglerd_exe_path = current_exe_dir
            .join("jugglerd.exe")
            .to_str()
            .unwrap()
            .to_string();

        // This should be allowed because it's jugglerd in the correct directory
        assert!(is_command_allowed_with_allowlist(&jugglerd_path, &allowlist));

        // This should also be allowed because it's jugglerd.exe in the correct directory
        assert!(is_command_allowed_with_allowlist(
            &jugglerd_exe_path,
            &allowlist
        ));
        ...
        assert!(!is_command_allowed_with_allowlist(
            "/usr/local/bin/jugglerd",
            &allowlist
        ));
        assert!(!is_command_allowed_with_allowlist(
            "/Users/username/path/to/jugglerd",
            &allowlist
        ));

        // Commands with arguments should NOT be allowed - we require exact matches
        assert!(!is_command_allowed_with_allowlist(
            "/Users/username/path/to/jugglerd --flag value",
            &allowlist
        ));

        // Simple jugglerd without path should be allowed
        assert!(is_command_allowed_with_allowlist("./jugglerd", &allowlist));
        assert!(is_command_allowed_with_allowlist("jugglerd", &allowlist));

        // These should NOT be allowed because they don't end with "/jugglerd"
        assert!(!is_command_allowed_with_allowlist(
            "/usr/local/bin/jugglerd-extra",
            &allowlist
        ));
        assert!(!is_command_allowed_with_allowlist(
            "/usr/local/bin/not-jugglerd",
            &allowlist
        ));
        assert!(!is_command_allowed_with_allowlist(
            "jugglerd-extra",
    ...
    fn test_windows_paths() {
        ...
        let test_paths = vec![
            ...
            r"C:\Users\MaxNovich\Downloads\Juggler-1.0.17\resources\bin\uvx.exe",
            // Path with different casing
            r"C:\Users\MaxNovich\Downloads\Juggler-1.0.17\Resources\Bin\uvx.exe",
            // Path with forward slashes
            r"C:/Users/MaxNovich/Downloads/Juggler-1.0.17/resources/bin/uvx.exe",
            // Path with spaces
            r"C:\Program Files\Juggler 1.0.17\resources\bin\uvx.exe",
            // Path with version numbers
            r"C:\Users\MaxNovich\Downloads\Juggler-1.0.17-block.202504072238-76ffe-win32-x64\Juggler-1.0.17-block.202504072238-76ffe-win32-x64\resources\bin\uvx.exe",
        ];
        ...
        let invalid_paths = vec![
            ...
            r"C:\Users\MaxNovich\Downloads\Juggler-1.0.17\resources_modified\bin\uvx.exe",
            // Path with extra components
            r"C:\Users\MaxNovich\Downloads\Juggler-1.0.17\resources\bin\extra\uvx.exe",
        ];
    ...
    fn test_windows_uvx_path() {
        ...
        let windows_path = r"C:\Users\MaxNovich\Downloads\Juggler-1.0.17-block.202504072238-76ffe-win32-x64\Juggler-1.0.17-block.202504072238-76ffe-win32-x64\resources\bin\uvx.exe";
        let cmd = format!("{} mcp_snowflake@latest", windows_path);

        // This should be allowed because it's a valid uvx command in the Juggler resources/bin directory
        assert!(is_command_allowed_with_allowlist(&cmd, &allowlist));

        // Test with different casing and backslashes
        let windows_path_alt = r"c:\Users\MaxNovich\Downloads\Juggler-1.0.17-block.202504072238-76ffe-win32-x64\Juggler-1.0.17-block.202504072238-76ffe-win32-x64\Resources\Bin\uvx.exe";
...
"#;
        ...
        env::set_var("JUGGLER_ALLOWLIST", format!("{}{}", server_url, server_path));
        ...
        // Clean up
        env::remove_var("JUGGLER_ALLOWLIST");
    ...
    fn test_allowlist_bypass() {
        ...
        env::set_var("JUGGLER_ALLOWLIST_BYPASS", "true");
        ...
        // Clean up
        env::remove_var("JUGGLER_ALLOWLIST_BYPASS");
        ...
        let test_with_allowlist_and_bypass = |bypass_value: &str, expected: bool| {
            if bypass_value.is_empty() {
                ...
                env::set_var("JUGGLER_ALLOWLIST_BYPASS", bypass_value);
            }

            // This is what we're testing - a direct call that simulates what happens in is_command_allowed
            let result = if let Ok(bypass) = env::var("JUGGLER_ALLOWLIST_BYPASS") {
            ...
            assert_eq!(
                result,
                expected,
                "With JUGGLER_ALLOWLIST_BYPASS={}, expected allowed={}",
                if bypass_value.is_empty() {
                    "not set"
                } else {
                    bypass_value
