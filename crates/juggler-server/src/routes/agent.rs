...
use juggler::config::PermissionManager;
...
async fn get_versions() -> Json<VersionsResponse> {
    let versions = ["juggler".to_string()];
    let default_version = "juggler".to_string();
    ...
    let mut tools: Vec<ToolInfo> = agent
        ...
        .map(|tool| {
            let permission = permission_manager
                ...
                .or_else(|| {
                    if juggler_mode == "smart_approve" {
                        permission_manager.get_smart_approve_permission(&tool.name)
                    } else if juggler_mode == "approve" {
