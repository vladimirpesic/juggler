...
use juggler::config::APP_STRATEGY;
use juggler::config::{extensions::name_to_key, PermissionManager};
use juggler::config::{ExtensionConfigManager, ExtensionEntry};
use juggler::model::ModelConfig;
use juggler::providers::base::ProviderMetadata;
use juggler::providers::pricing::{get_all_pricing, get_model_pricing, refresh_pricing};
use juggler::providers::providers as get_providers;
use juggler::{agents::ExtensionConfig, config::permission::PermissionLevel};
...
pub async fn get_extensions(
    ...
    match ExtensionConfigManager::get_all() {
        ...
        Err(err) => {
            if err
                .downcast_ref::<juggler::config::base::ConfigError>()
                .is_some_and(|e| matches!(e, juggler::config::base::ConfigError::DeserializeError(_)))
            {
...
pub async fn init_config(
    ...
    match juggler::config::base::load_init_config_from_workspace() {
...
pub async fn backup_config(
    ...
    let config_dir = choose_app_strategy(APP_STRATEGY.clone())
        .expect("juggler requires a home dir")
        .config_dir();
...
pub async fn get_current_model(
    ...
    let current_model = juggler::providers::base::get_current_model();
...
mod tests {
    ...
    async fn test_read_model_limits() {
        let test_state = AppState::new(
            Arc::new(juggler::agents::Agent::default()),
            "test".to_string(),
        )
        .await;
        let sched_storage_path = choose_app_strategy(APP_STRATEGY.clone())
            ...
            .data_dir()
            .join("schedules.json");
        let sched = juggler::scheduler_factory::SchedulerFactory::create_legacy(sched_storage_path)
            .await
            .unwrap();
        ...
        let limits: Vec<juggler::model::ModelLimitConfig> =
