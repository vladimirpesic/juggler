...
fn get_log_directory() -> Result<PathBuf> {
    ...
    // - macOS/Linux: ~/.local/state/juggler/logs/server
    // - Windows:     ~\AppData\Roaming\juggler\data\logs\server
...
pub fn setup_logging(name: Option<&str>) -> Result<()> {
    ...
    let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        ...
        EnvFilter::new("")
            ...
            // Set juggler-server to INFO
            .add_directive("juggler_server=info".parse().unwrap())
